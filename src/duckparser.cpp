...
namespace duckparser {
    ...
    void parse(const char* str, size_t len) {
        ...
        while (n) {
            ...
            else if (compare1(cmd->str, cmd->len, "STRDELAY", CASE_SENSETIVE) || compare1(cmd->str, cmd->len, "STRINGDELAY", CASE_SENSETIVE)) {
                strDelay = toInt(line_str, line_str_len);
            }
