    ...
    ws_send("ls", function(csv) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (fileName.length > 0) {
          ...
          tableHTML += "<div class='script-list-entry' onclick=\"selectSingle(this); read('" + fileName + "')\">";
          tableHTML += "<h5>" + fileName + "</h5>\n";
          tableHTML += "</div>\n";
        }
      }
      tableHTML += "</tbody>\n";
  ...
  ws_send("ls", function(csv) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (fileName.length > 0) {
        ...
        tableHTML += "<td>\n";
        tableHTML += "<button class=\"primary subtle\" onclick=\"read('" + fileName + "')\">edit</button>\n";
        tableHTML += "<button class=\"warn subtle\" onclick=\"run('" + fileName + "')\">run</button>\n";
        tableHTML += "</tr>\n";
      }
    }
    tableHTML += "</tbody>\n";
...
function testTable() {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (fileName.length > 0) {
      ...
      //tableHTML += "<button class=\"subtle\" onclick=\"run('" + fileName + "')\">Upload</button>\n";
      tableHTML += "</div>\n";
    }
  }
  //tableHTML += "</tbody>\n";
...
function selectSingle(o) {
  ...
  o.classList.add('sel');
  showHoverMenu();
  document.getElementById('runScriptName').innerText = document.querySelector('.sel').innerText;
}

function hideAll() {
  Array.from(document.querySelectorAll('.sel')).forEach(
    (el) => el.classList.remove('sel')
  );
}

function showHoverMenu() {
  var hb = document.querySelector('.hover-bar');
  hb.classList.remove("hide");
  setTimeout(function() {hb.style.bottom = "0"}, 400);
}

function hideHoverMenu() {
  var hb = document.querySelector('.hover-bar');
  hb.style.bottom = "-100px";
  setTimeout(function() {hb.classList.add("hide")}, 400);
}
...
window.addEventListener("load", function() {
  ...
  E("scriptsReload").onclick = function() { update_file_list(); hideAll(); hideHoverMenu()};
  //E("format").onclick = format;
  E("stop").onclick = stopAll;

  

  E("editorReload").onclick = function() {reloadEditor()}
  ...
  E("editorAutorun").onclick = function() {
    if (confirm("Run this script with the action button?\nYou can disable it in the settings."))
